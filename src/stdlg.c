...
INT_PTR CALLBACK UpdateCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		// Disable update controls on NT5 (port)
		EnableWindow(GetDlgItem(hDlg, IDC_CHECK_NOW),
			(WindowsVersion.Version >= WINDOWS_VISTA) && (freq != 0));
		EnableWindow(hFrequency, WindowsVersion.Version >= WINDOWS_VISTA);
		...
		if (WindowsVersion.Version < WINDOWS_VISTA) {
			// Experimental NT5 GPT option (port)
			SetWindowTextU(GetDlgItem(hDlg, IDS_INCLUDE_BETAS_TXT), lmprintf(MSG_355));
			Reposition(hDlg, IDS_INCLUDE_BETAS_TXT, -1, 0, 42);
			Reposition(hDlg, IDC_INCLUDE_BETAS, -1, 42, -42);
			EnableWindow(hBeta, TRUE);
			IGNORE_RETVAL(ComboBox_SetCurSel(hBeta, ReadSettingBool(SETTING_EXPERIMENTAL_GPT) ? 0 : 1));
		} else {
	...
	case WM_COMMAND:
		...
		case IDC_UPDATE_FREQUENCY:
			...
			EnableWindow(hBeta, (WindowsVersion.Version < WINDOWS_VISTA) ? TRUE : ((freq >= 0) && is_x86_64));
			return (INT_PTR)TRUE;
		case IDC_INCLUDE_BETAS:
			if (HIWORD(wParam) != CBN_SELCHANGE)
				break;
			if (WindowsVersion.Version < WINDOWS_VISTA) {
				WriteSettingBool(SETTING_EXPERIMENTAL_GPT, ComboBox_GetCurSel(hBeta) == 0);
...
LPCDLGTEMPLATE GetDialogTemplate(int Dialog_ID)
{
	...
	if (wcscmp(L"Segoe UI Symbol", wBuf) == 0) {
		...
		const WCHAR* fontName = (WindowsVersion.Version == WINDOWS_2000) ? L"Tahoma" : L"Segoe UI";
